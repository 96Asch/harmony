...
@SuppressWarnings("all")public class EditSongModalSong_StringPage extends utils.PageServlet{
...
conversion();
loadArguments();
Object[] args=new Object[]{song_,header_};
this.args=args;
}
protected void initTemplateClass(){
templateservlet=new EditSongModalSong_String_Template();
}
...
combinedparams.append("{\"name\":\"").append("header").append("\", \"value\":\"").append(utils.HTMLFilter.filterEcmaWithinHTML(header_.toString())).append("\"},");
}
...
combinedparams.append("header").append("=").append(utils.HTMLFilter.filter(utils.URLFilter.filter(header_.toString()))).append("&");
}
...
combinedparams.append("<input type=\"hidden\" name=\"").append("header").append("\" value=\"").append(utils.HTMLFilter.filter(header_.toString())).append("\" />");
}
