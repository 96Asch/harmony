...
@SuppressWarnings("all")public class TestString_BandPage extends utils.PageServlet{
...
conversion();
loadArguments();
Object[] args=new Object[]{query_,band_};
this.args=args;
}
protected void initTemplateClass(){
templateservlet=new testString_Band_Template();
}
...
combinedparams.append("{\"name\":\"").append("band").append("\", \"value\":\"").append(org.apache.commons.lang3.StringEscapeUtils.escapeEcmaScript(band_.getNaturalId().toString())).append("\"},");
}
...
combinedparams.append("band").append("=").append(utils.HTMLFilter.filter(band_.getNaturalId().toString())).append("&");
}
...
combinedparams.append("<input type=\"hidden\" name=\"").append("band").append("\" value=\"").append(utils.HTMLFilter.filter(band_.getNaturalId().toString())).append("\" />");
}
...
if(band__id!=null){
band_=(webdsl.generated.domain.Band)utils.HibernateUtil.getCurrentSession().load(webdsl.generated.domain.Band.class,java.util.UUID.fromString(band__id));
}
}
...
if(parammap.get("band")!=null){
band__id=parammap.get("band");
}
}
