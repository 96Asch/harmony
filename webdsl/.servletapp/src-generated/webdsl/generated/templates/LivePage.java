...
@SuppressWarnings("all")public class LivePage extends utils.PageServlet{
...
conversion();
loadArguments();
Object[] args=new Object[]{setlist_,index_};
this.args=args;
}
protected void initTemplateClass(){
templateservlet=new Live_Template();
}
...
combinedparams.append("{\"name\":\"").append("index").append("\", \"value\":\"").append(utils.HTMLFilter.filterEcmaWithinHTML(index_.toString())).append("\"},");
}
...
combinedparams.append("index").append("=").append(utils.HTMLFilter.filter(utils.URLFilter.filter(index_.toString()))).append("&");
}
...
combinedparams.append("<input type=\"hidden\" name=\"").append("index").append("\" value=\"").append(utils.HTMLFilter.filter(index_.toString())).append("\" />");
}
...
try{
index_=Integer.valueOf(parammap.get("index"));
}
